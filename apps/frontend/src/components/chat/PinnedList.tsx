      ...
      <div className="flex flex-col gap-0.5">
        {pins.map((m: MessageDTO) => (
          <div
            ...
            role="button"
            tabIndex={0}
            onClick={() => serverId && navigate(`/channels/${serverId}/${channelId}?message=${m.id}`)}
            onKeyDown={(e) => {
              if ((e.key === "Enter" || e.key === " ") && serverId) {
                e.preventDefault();
                navigate(`/channels/${serverId}/${channelId}?message=${m.id}`);
              }
            }}
            className="lx-focus group cursor-pointer rounded-lg px-2 py-2 hover:bg-base-600"
          >
            <div className="flex items-baseline gap-1.5">
              ...
              {canManage && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    togglePin.mutate({ messageId: m.id, pinned: false });
                  }}
