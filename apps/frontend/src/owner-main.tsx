...
import { isAdmin } from "./lib/platformRole";
...
function OwnerRoot() {
  ...
  if (!isAdmin(user.platformRole)) return <NotAuthorized />;
  return <OwnerApp />;
}
  ...
  return (
    <div className="flex h-app items-center justify-center bg-base-900 p-6">
      <div className="w-full max-w-sm rounded-xl border border-hairline bg-base-800 p-6 text-center">
        ...
        <h1 className="font-display text-lg text-signal">Admin access required</h1>
        <p className="mt-2 text-sm text-signal-dim">
          This console is for admins and above. Moderators work from the staff suite in the main
          app, which is where the review queues live.
        </p>
        <button
          type="button"
